 ...
 */
export const removeQuads: AstIndir<'removeQuads', Algebra.Operation, [Algebra.Operation]> = {
  name: 'removeQuads',
  fun: ({ SUBRULE }) => (_, op) =>
    <typeof op>SUBRULE(removeQuadsRecursive, op, []),
};
 ...
 */
export const removeQuadsRecursive: AstIndir<
...
> = {
  ...
  fun: ({ SUBRULE }) => ({ factory }, op, graphs) => {
    if (Array.isArray(op)) {
      return op.map(sub => <typeof sub> SUBRULE(removeQuadsRecursive, sub, graphs));
    }
    ...
    for (const key of Object.keys(op)) {
      ...
      result[key] = SUBRULE(removeQuadsRecursive, op[key], newGraphs);
    ...
    if (graphNameSet.length > 0) {
      ...
      if (graphNameSet.length === 1 && ![ types.PROJECT, types.SERVICE ].includes(op.type)) {
        ...
        return SUBRULE(splitBgpToGraphs, op, keyGraphs.patterns);
      } else {
 ...
 */
export const splitBgpToGraphs: AstIndir<
  'splitBgpToGraphs',
